...
function M.setup(wk, config, options)
...
function M.run(trigger, mode, buf)
  ...
  local suggestions = vim.fn.spellsuggest(word, M.opts.suggestions or 20,
                                          bad[2] == "caps" and 1 or 0)
  ...
  for i, label in ipairs(suggestions) do
    ...
    table.insert(items,
                 { key = key, label = label, fn = function() vim.cmd("norm! ciw" .. label) end })
  end
  return items
end
